...
bool sy_2::OpenFile3()
{
	...
	//*第 2 步： 将图像数据读入数据集中，检查是否有金字塔文件，若无则构建金字塔文件
	...
	//int toplayersize = 16384;//设置参考图像保存金字塔文件的顶层大小为128*128，根据函数调用的值的大小，动态设置顶层金字塔的大小
	//int toplayersize = 65536;//设置参考图像保存金字塔文件的顶层大小为256*256，根据函数调用的值的大小，动态设置顶层金字塔的大小
	int toplayersize = 262144;//设置参考图像保存金字塔文件的顶层大小为512*512，根据函数调用的值的大小，动态设置顶层金字塔的大小
	pyramid3.sCheckPyramid(toplayersize);//构建图像金字塔，并没有保存各层金字塔图层
	...
	//*第 4 步：保存金字塔图层，或者利用Opencv库函数对图像进行降采样后，保存1/2,1/4,1/8降采样的结果
	...
	//删除dir文件夹
	/*const char *file_name = dir + "\\*";*/
	/*delete_file(file_name);*/


	//判断该文件夹是否存在，若存在，删除重新简历；若不存在，则创建该文件夹
	//if(_access(dir, 0) != -1)  
	//{  
		//cout << "文件夹已存在" <<endl;  
		//rmdir(dir);
		...
		//DeleteFolder(dir);
	//	string dirr = dir;
	//	bool falgfile = RemoveDirectory(dirr.c_str());
		//system("rd dir");

	//}  
	//无论是否存在，都要创建该文件夹
	bool rmflag = remove_directory(dir);
// 	if (rmflag)
// 	{
// 		system("pause");
// 	}
	...
	//	char filepath[1024] = "";			//filepath[1024]:用以保存函数返回的金字塔每一层图像路径
	//	pyramid3.SaveAllBand(i,filepath);//,pyramid1.InFile);
	//	refLayer.push_back(filepath);	//refLayer[i+1]:参考图像金字塔第i层图像路径；其中refLayer[1]保存第0层(原图像素的1/4 * 1/4)
	//}
	...
	scr = cvLoadImage(refImg[1].c_str(),-1);//对于较大的图像，这一步可能会失败
	rfimg.push_back(scr);
	CvSize dst_cvsize;
	double scale;
	...
	//人性化提示：如针对打开的图像的大小，选择不同的降采样方式，或者选择不同的配准处理方式
...
bool sy_2::OpenFile4()
{
	...
	//设置待配准图像顶层金字塔图像大小为64*64，这样就使得对于较小的目标图像仍然能够保存至少三层金字塔文件
	int toplayersize = 4096;
	pyramid4.sCheckPyramid(toplayersize);
...
/*	} */
	...
	//	char filepath[1024] = "";			//filepath[1024]:用以保存函数返回的金字塔每一层图像路径
	//	pyramid4.SaveAllBand(i,filepath);//,pyramid1.InFile);
	//	senLayer.push_back(filepath);	//refLayer[i+1]:参考图像金字塔第i层图像路径；其中refLayer[1]保存第0层(原图像素的1/4 * 1/4)
	//}
...
// 	ResizeImage(senImg[1].c_str(),downsen.c_str(),downrate);
	...
	fenImage.clear();
	//*创建结果图像所在文件夹*//
	string dir = refImg[0] + "DFenKuaiResultFiles"; ///在参考图像文件夹下创建文件夹，保存小图和大图的配准结果
	//if(_access(dir.c_str(), 0) != -1)   
		//cout << "文件夹已存在" <<endl;  
	//else  
	//{  
...
// 		cout << "文件夹不存在，创建文件夹" << endl;  
	...
	stream << downScale_qt;
	downqt = stream.str();
	string downResult = dir1 +downqt +"DownResult.jpg";	//图像匹配结果图
	string Result = dir1 + downqt +"DResult.jpg";	//大图匹配结果图
	resultImg.clear(); 
	resultImg.push_back(dir1);				//resultImg[0]:结果图像所在文件夹
...
//	vector<int>ptpairs2;
	...
	if( H )
	{
		...
		double ppx,ppy,RR02,RR12,RRa,RRb,rotate,bighx,bighy;
		ppx=nn*ww;
		ppy=mm*hh;
		...
		p1.x=(snimg[0]->width)*R11/downrate+bighx;  p1.y=R12*(snimg[0]->width)/downrate+bighy; 
...
void sy_2::FenKuai_SURF()
{
	...
	if (flag_ImageSize == 0)//参考图像小于4000，调用Opencv裁剪分块的方法
	{
		Fenkuai_Registration_Small();
	} 
	else//参考图像大小大于4000，则采用GDAL分块方式进行分块匹配
	{
		Fenkuai_Registration_Big();
	}
...
void Fenkuai_Registration_Small()
{
	...
	string Result = dir1 + "DResult.jpg";	//大图匹配结果图
	resultImg.clear(); 
	resultImg.push_back(dir1);				//resultImg[0]:结果图像所在文件夹
	resultImg.push_back(downResult);	//resultImg[1]：降采样图像配准结果路径
	resultImg.push_back(Result);		//resultImg[2]：大图配准结果
